...
function updateTraceUI(w) {
  ...
  $('finishLetter').innerHTML = letterIndex === letters.length-1 ? '完成这个单词 <span>⭐</span>' : '这个字母写好了 <span>→</span>';
...
function endStroke(event) {
  ...
  $('finishLetter').disabled = inkDistance < 80;
}
...
$('clearWrite').addEventListener('click',clearPad);
...
$('finishLetter').addEventListener('click', () => {
  if ($('finishLetter').disabled) return;
  const w = currentWord();
  if (letterIndex < w.word.length-1) {
    letterIndex += 1;
