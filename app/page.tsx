...
export default function Home(){
 const today=isoDate(new Date()); const [date,setDate]=useState(today); const [amount,setAmount]=useState(3); const [current,setCurrent]=useState(0); const [done,setDone]=useState<Record<string,string[]>>({}); const [recording,setRecording]=useState(false); const [result,setResult]=useState(false); const [settings,setSettings]=useState(false); const timer=useRef<ReturnType<typeof setTimeout>|null>(null);
 useEffect(()=>{try{const saved=JSON.parse(localStorage.getItem(key)||"{}");setDone(saved.done||{});setAmount(saved.amount||3)}catch{}},[]);
 useEffect(()=>{localStorage.setItem(key,JSON.stringify({done,amount}))},[done,amount]);
 ...
 useEffect(()=>{setCurrent(0);setResult(false);setRecording(false)},[date,amount]);
 ...
 return <main>
  ...
  <article className="lesson-card"><div className="step-line"><span className="on">1 听一听</span><i/><span className={recording||result?"on":""}>2 读一读</span><i/><span className={result?"on":""}>3 得星星</span></div><div className={`picture ${item.tone}`}><span>{item.emoji}</span><div className="bubble">{item.bubble}</div></div><div className="word-heading"><h2>{item.word}</h2><p>{item.phonetic} · {item.zh}</p></div><button className="listen" onClick={()=>speak(item.word)}><span>🔊</span><b>听单词发音</b><small>点我听一听</small></button><p className="example"><span>例句</span> {item.sentence} <button onClick={()=>speak(item.sentence)}>🔈</button></p>
